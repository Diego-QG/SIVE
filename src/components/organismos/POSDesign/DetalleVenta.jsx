    ...
    <Overlay $visible={open}>
      <Modal>
        <ModalHeader>
          ...
          <CloseButton type="button" onClick={onClose} disabled={loading} aria-label="Cerrar detalles">
            ✕
          </CloseButton>
        </ModalHeader>
              ...
              <MainGrid>
                <LeftColumn>
                            ...
                            <tr key={`${item.nombre_material}-${index}`}>
                              <td>
                                <div className="item-desc">
                                  ...
                                  <small>
                                    {[item.tipo_contenido, item.nivel, item.subnivel, item.curso]
                                      .filter(Boolean)
                                      .join(" • ")}
                                  </small>
                                </div>
                              ...
                              <td className="text-right">{mostrarConGuion(item.precio_unitario)}</td>
                              <td className="text-right font-bold">{mostrarConGuion(item.subtotal)}</td>
                            </tr>
                          ))}
                        </tbody>
                      </ItemsTable>
                  ...
                  <Section>
                    ...
                    {cuotas.length === 0 ? (
                      <EmptySection>Esta venta no tiene cuotas configuradas.</EmptySection>
                    ) : (
                      <CuotasContainer>
                          ...
                          <CuotaRow key={`cuota-${index}`}>
                            <div className="cuota-header">
                              <div className="cuota-title">
                                <span className="cuota-number">{cuota.nro_cuota}</span>
                                <div className="cuota-info">
                                  <strong>Vence: {mostrarConGuion(cuota.fecha_vencimiento)}</strong>
                                  <small>{mostrarConGuion(cuota.estado)}</small>
                                </div>
                              </div>
                              <div className="cuota-amount">
                                <small>Saldo: {mostrarConGuion(cuota.saldo)}</small>
                            ...
                            {Array.isArray(cuota.pagos) && cuota.pagos.length > 0 && (
                              <PagosList>
                                {cuota.pagos.map((pago, pagoIdx) => (
                                  <div className="pago-wrapper" key={`pago-${pagoIdx}`}>
                                    <div className="pago-item">
                                      <div className="pago-info">
                                        <strong>{mostrarConGuion(pago.fecha_pago)}</strong>
                                        <small>
                  ...
                  <SideCard>
                    ...
                    <FinancialList>
                      {montosVenta.map((monto, i) => (
                        <div key={i} className={`financial-row ${monto.highlight ? 'highlight' : ''}`}>
                    ...
                    <SideCard>
                      ...
                      <AlertsList>
                        {descuentos.map((dcto, index) => (
                          <AlertItem key={`${dcto.codigo}-${index}`} type="descuento">
                            <div className="flex-between">
                                <strong>{mostrarConGuion(dcto.nombre)}</strong>
                                <span className="pill">{mostrarConGuion(dcto.codigo)}</span>
                            </div>
                            <small>{mostrarConGuion(dcto.fecha_aplicado)}</small>
                            <small>Aplicado por: {mostrarConGuion(dcto.aplicado_por)}</small>
                            <strong className="monto-dcto">-{mostrarConGuion(dcto.monto_descuento)}</strong>
                          </AlertItem>
                        ))}
                      </AlertsList>
                    </SideCard>
                  ...
                  <SideCard>
                    ...
                    <DocenteInfo>
                      ...
                      <div className="docente-details">
                        ...
                        <small>{mostrarConGuion(docenteData?.docente_nro_doc)}</small>
                      </div>
                      <ContactInfo>
                        ...
                        <div className="contact-row">
                          <span>Tipo:</span>
                          <strong>{mostrarConGuion(docenteData?.docente_tipo_ingreso)}</strong>
                        </div>
                      </ContactInfo>
                    </DocenteInfo>
                  </SideCard>
...
const PagosList = styled.div`
  ...
  .pago-item {
    ...
    .pago-info {
      ...
      small { font-size: 0.75rem; color: rgba(${({ theme }) => theme.textRgba}, 0.6); }
    }
...
const DocenteInfo = styled.div`
  ...
  .docente-details {
    ...
    small { color: rgba(${({ theme }) => theme.textRgba}, 0.6); }
  }
`;
...
const ContactInfo = styled.div`
  ...
  .contact-row {
    ...
    justify-content: space-between;
    font-size: 0.9rem;
    span { color: rgba(${({ theme }) => theme.textRgba}, 0.6); }
...
const AlertItem = styled.div`
  padding: 10px;
  border-radius: 8px;
  background: ${props => props.type === 'incidente' ? 'rgba(245, 78, 65, 0.1)' : props.type === 'descuento' ? 'rgba(83, 178, 87, 0.1)' : 'rgba(243, 210, 12, 0.1)'};
  border: 1px solid ${props => props.type === 'incidente' ? 'rgba(245, 78, 65, 0.2)' : props.type === 'descuento' ? 'rgba(83, 178, 87, 0.2)' : 'rgba(243, 210, 12, 0.2)'};
  display: flex;
  flex-direction: column;
  gap: 4px;
...
const Step = styled.div`
  ...
  .step-dot {
    ...
    height: 16px;
    border-radius: 50%;
    background: ${props => props.status ? props.theme.verde : `rgba(${props.theme.textRgba}, 0.2)`};
  ...
  .step-content {
    ...
    flex-direction: column;
    margin-top: -2px;
    small { font-size: 0.8rem; color: rgba(${({ theme }) => theme.textRgba}, 0.6); }
